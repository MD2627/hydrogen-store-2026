...
import { BlogGridFromPage } from '~/components/BlogGridFromPage';
import { BLOGS_QUERY } from '~/routes/blogs.$blogHandle._index';
  ...
  throw new Response("Not Found", { status: 404 });
    ...
    try {
        const { blog } = await storefront.query(BLOGS_QUERY, {
            variables: {
                blogHandle: 'moissanite-guidance',
                first: 20
            },
        });

        console.log('Moissanite Blog Check:', blog ? `Found: ${blog.title}` : 'Not Found', 'Articles:', blog?.articles?.nodes?.length);

        if (!blog) {
            console.error('Moissanite blog missing!');
            throw new Response('Blog not found', { status: 404 });
        }

        return { blog };
    } catch (error) {
        console.error('Error loading Moissanite blog:', error);
        throw error;
    }
}

/* ---------------- PAGE ---------------- */
export default function MoissaniteBlog() {
    const { blog } = useLoaderData();

    return (
        <div className="moissanite-blog-page">
            <div className="blogs-outer-title">
                <h2>{blog.title}</h2>
