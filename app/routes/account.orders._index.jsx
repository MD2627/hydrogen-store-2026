...
function OrderItem({order}) {
  ...
  return (
    <div className="order-item">
      ...
      <div>Date: {new Date(order.processedAt).toDateString()}</div>
      {order.confirmationNumber && (
        <div>Confirmation: {order.confirmationNumber}</div>
      )}
      <div>Status: {order.financialStatus}</div>
      {fulfillmentStatus && <div>Fulfillment: {fulfillmentStatus}</div>}
      <div>Total: <Money data={order.totalPrice} /></div>
      <div>
        <Link to={`/account/orders/${btoa(order.id)}`}>View Order Details →</Link>
      </div>
    </div>
  );
}
