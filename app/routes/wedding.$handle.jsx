...
export const meta = ({ data }) => {
  return [
    ...
    {
      rel: 'canonical',
      href: `/wedding/${data?.product.handle}`,
    },
  ];
};
          ...
          <div className="back-navigation-wrapper">
            <button
              onClick={() => {
                ...
                // Detect if this is a men's or women's product based on tags or title
                // IMPORTANT: Check for 'women' FIRST because 'women' contains 'men'!
                const productText = (product.tags?.join(' ') + ' ' + product.title).toLowerCase();
                const isWomensProduct = productText.includes('women') || productText.includes('female');
                const isMensProduct = !isWomensProduct && (productText.includes('men') || productText.includes('male'));

                // Set the appropriate tab (default to women if neither is detected)
                params.set('tab', isMensProduct ? 'men' : 'women');
                ...
                selectedVariant?.selectedOptions?.forEach(option => {
                  ...
                  } else if (optionName.includes('setting') && !optionName.includes('setting')) {
                ...
                navigate(queryString ? `/wedding?${queryString}` : '/wedding');
              }}
